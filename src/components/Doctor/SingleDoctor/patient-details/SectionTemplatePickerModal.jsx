...
const SectionTemplatePickerModal = ({
  isOpen,
  onClose,
  sectionKey,
  sectionLabel,
  ...
  useEffect(() => {
    if (!isOpen || !sectionKey) return;
    setError(null);
    setLoading(true);
    visitTemplatesHelper
      .listSectionTemplates(sectionKey)
      .then(setTemplates)
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            Wybierz szablon{sectionLabel ? ` – ${sectionLabel}` : ""}
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          {!loading && !error && templates.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-4">
              Brak szablonów dla tej sekcji. Utwórz je w Ustawienia → Szablony dokumentów.
            </p>
          )}
...
export default SectionTemplatePickerModal;
