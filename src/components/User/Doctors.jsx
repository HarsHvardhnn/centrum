...
import { FaInstagram, FaFacebookF, FaLinkedinIn, FaTimes, FaCalendarAlt } from "react-icons/fa";
...
export default function Doctors({
  ...
  const fetchDoctorProfile = async (doctorId) => {
    try {
      const response = await axios.get(`http://localhost:5000/docs/profile/${doctorId}`);
  ...
  const fetchAvailableSlots = async (doctorId, date) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/docs/schedule/available-slots/${doctorId}?date=${date}`
      );
      if (response.data.success) {
  ...
  const handleConfirmAppointment = async () => {
    ...
    try {
      ...
      const appointmentData = {
        date: selectedDate,
        department: selectedDoctor.department._id || selectedDoctor.department.id,
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-1/3">
                  <div className="bg-gray-100 p-4 rounded-lg">
                    ...
                    <p className="text-gray-700 font-medium">{selectedDoctor.department.name}</p>
                    
                    {doctorProfile ? (
                      <div className="mt-4 text-left">
                        {doctorProfile.experience && (
                          <p className="text-sm mt-2">
                            <span className="font-semibold">Doświadczenie:</span> {doctorProfile.experience} lat
                          </p>
                        )}
                        {doctorProfile.education && (
                          <p className="text-sm mt-2">
                            <span className="font-semibold">Edukacja:</span> {doctorProfile.education}
                          </p>
                        )}
                        {doctorProfile.languages && (
                          <p className="text-sm mt-2">
                            <span className="font-semibold">Języki:</span> {doctorProfile.languages.join(', ')}
                          </p>
                        )}
                        {selectedDoctor.consultationFee && (
                          <p className="text-sm font-medium mt-2">
                            <span className="font-semibold">Cena konsultacji:</span> {selectedDoctor.consultationFee} zł
                          </p>
                        )}
                      </div>
                    ) : (
                ...
                <div className="md:w-2/3">
                  <h4 className="text-lg font-semibold mb-4">Wybierz termin wizyty</h4>
                        ...
                        return (
                          ...
                          >
                            <div className="font-medium">
                              {dayDate.toLocaleDateString('pl-PL', { weekday: 'short' })}
                            </div>
                  ...
                  <div className="mb-6">
                    ...
                    {slotsLoading ? (
                      ...
                      <div className="text-center py-6 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-gray-700">Brak dostępnych terminów w wybranym dniu</p>
                      </div>
                    )}
                  </div>
                  ...
                  {selectedSlot && (
                    <div className="border-t pt-6 mt-6">
                      <h4 className="text-lg font-semibold mb-4">Dane pacjenta</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-3 py-2 border ${formErrors.name ? 'border-red-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-1 focus:ring-teal-500`}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleInputChange}
                            className={`w-full px-3 py-2 border ${formErrors.email ? 'border-red-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-1 focus:ring-teal-500`}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-3 py-2 border ${formErrors.phone ? 'border-red-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-1 focus:ring-teal-500`}
                          ...
                          {formErrors.phone ? (
                            ...
                            <p className="text-gray-500 text-xs mt-1">Format: 9 cyfr bez spacji i znaków specjalnych</p>
                          )}
                        </div>
