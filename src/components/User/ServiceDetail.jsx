  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 p-6 mx-auto pt-16 max-w-6xl">
        ...
        <div className=" border border-neutral-200 rounded-lg overflow-hidden">
          {servicesData.map((item) => (
            <Link
              ...
              key={item.id}
              className={`flex items-center gap-2 cursor-pointer px-8 py-7 ${
      ...
      <div className="w-full md:w-3/4">
        ...
        <h2 className="text-4xl font-serif font-bold text-main mt-6">
          {service.title}
        </h2>

        <ul className="grid grid-cols-2 gap-2 mt-4 text-xl text-neutral-900">
        ...
        <p className="mt-4 text-neutral-900 text-lg">{service.description}</p>
      </div>
    </div>
  );
};
